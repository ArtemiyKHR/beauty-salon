...
document.addEventListener("DOMContentLoaded", function () {
  ...
  async function formSend(e) {
    ...
    let error = formValidate(form); // Валидируем форму
    ...
    if (error === 0) {
      ...
      try {
        ...
        if (response.ok) {
          showMessage(result, true); // Успешное сообщение от сервера
          form.reset(); // Сбрасываем форму
        } else {
          showMessage(result || "Ошибка отправки формы.", false);
        }
      } catch (error) {
        showMessage("Ошибка подключения к серверу.", false);
        popupContent.classList.remove("_sending");
      }
    } else {
  ...
  // Тесты для проверки значений
  function emailTest(input) {
    return !/^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,8})+$/.test(input.value);
  }

  function nameTest(input) {
    return /\d/.test(input.value); // Имя не должно содержать цифры
  }
